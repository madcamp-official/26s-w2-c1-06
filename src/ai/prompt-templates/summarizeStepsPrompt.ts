...
function describeStep(step: StepInput, index: number): string {
  ...
  step.events.forEach((event, i) => {
    ...
    if (event.result_content) {
      ...
      lines.push(`     ${label}: ${truncate(event.result_content, 300)}`)
    }
  })
...
// 활동 탭의 "바뀐 구조와 변경사항"에 실시간으로 쌓이는 진행 로그 전용 — 해설 없이
// "방금 한 일" 초단문 요약과, 이미 결정론적으로 뽑힌 대표 코드에 대한 정형화된
// 3줄 설명을 만든다.
//
// snippet/filePath/lang은 여기서 만드는 게 아니다 — step-worker가 실제 old_string/
// new_string에서 미리 추출해 "이미 확정된 사실"로 준다. AI는 코드를 다시 타이핑하지
// 않고, 주어진 코드를 보고 설명/중요한 이유/학습 포인트만 채운다(코드를 잘못
// 옮겨적을 위험을 원천 차단).
export function buildSummarizeStepsPrompt(steps: StepInput[], skillLevel: SkillLevel): string {
  ...
  return [
    ...
    '  - 좋은 예: "삭제 버튼 컴포넌트가 생성되어 MonitoringControl에 연결됐습니다" (결과/과거형)',
    ...
    '  - explanation: 이 코드가 무엇인지(무엇이 바뀌었는지) 한 문장.',
    '  - importance: 이 코드가 지금 중요한 이유 한 문장(재사용/연결될 코드, 흔한 실수 포인트,',
    '    또는 이전 시도의 실패/막힘과의 관계 중 가장 그럴듯한 관점으로).',
    '  - application: 이 코드를 보고 배울 점/앞으로 어떻게 적용할지 한 문장(학습 포인트).',
    '  - conceptTags: 이 코드와 관련된 프로그래밍 개념 해시태그 1~3개(짧은 명사구, # 없이).',
    '  - 네 필드 모두 반드시 채워라(비워두지 말 것).',
...
export const SUMMARIZE_STEPS_RESPONSE_SCHEMA: Schema = {
  type: Type.ARRAY,
  items: {
    type: Type.OBJECT,
    properties: {
